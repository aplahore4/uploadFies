...
$(document).ready(function () {
    ...
    $(".topBarNotificationIcon").on("click", function () {
        ...
        if (loggedInUserCompleteDetail.LicencePackageDetail.packageStatus != "Purchased") {
            if ($("#chkBoxSingleUserReoccurringPayment").is(":checked")) {
                $("#chkBoxSingleUserReoccurringPaymentImg").attr("src", "https://chartexpo.com/ChartExpoForGoogleSheetAddin/Images/v4/checkbox-active.png");
                $(".btnIndividualAutoRenewSubscription").css("display", "table-cell");
                $("#btnIndividualUserBuyNow").css("display", "none");
                $("#chkBoxSingleUserReoccurringPayment").attr('checked', true);
            }
            else {
                $("#chkBoxSingleUserReoccurringPaymentImg").attr("src", "https://chartexpo.com/ChartExpoForGoogleSheetAddin/Images/v4/checkbox-deactive.png");
                $(".btnIndividualAutoRenewSubscription").css("display", "none");
                $("#btnIndividualUserBuyNow").css("display", "table-cell");
                $("#chkBoxSingleUserReoccurringPayment").attr('checked', false);
            }
        }
        else {
    ...
    $(".topBarNotificationIcon").hover(function () {
        if (selectedScreen != "SubscriptionScreen" && selectedScreen != "PricingScreen") {
            var html = $('.subscriptionScreen').html();
            $('#tooltipSubscriptionScreen').html('');
    ...
    $('#tooltipSubscriptionScreen').on("mouseleave", function () {
        if (SubscriptionDomain == "Individual") {
            ...
            $('.subscriptionScreen').html('');
            $('.subscriptionScreen').append(html);
        }
        else {
            $('.teamUsersContainerSubscribeScreen').show();
        }
    ...
    $('#divTestDrawChart').click(function () {
        ...
        if (selectedChart == "ParetoGroupedChart" || selectedChart == "ParetoGroupedHorizontalChart") {
            imagePath = "https://apps.polyvista.com/GooglesheetFeb2021/scripts/polyvista/new192/" + selectedChart + ".png";
        }
    ...
    $('#info-icon-dimension').hover(function (event) {
        ...
        if (chartRuleObject.MaxDim > 1) {
            ...
            rearrangeColumns = '<br><br> Rearrange ' + dimension.toLowerCase() + ' with a drag-n-drop';
            only = "";
        }
    ...
    $('#info-icon-metric').hover(function (event) {
        ...
        var displayName = chartRuleObject.ChartDisplayName;
        var only = "only ";
        //var rearrangeColumns = '<br><br> You can rearrange ' + metric.toLowerCase() + 's with a drag-n-drop';
    ...
    $("#myChartsThumbnailsDiv").on("click", ".insertChartIntoSheet", function () {
        ...
        AddScript("https://apps.polyvista.com/GooglesheetFeb2021/scripts/polyvista/new223/ChartExpo.js", "ChartExpo", runInsertChartImageIntoSheetMethod, true);
...
function insertChartSampleDataIntoSheet(openedFromSource) {
    ...
    if (selectedChart == "ParetoGroupedChart" || selectedChart == "ParetoGroupedHorizontalChart") {
        imagePath = "https://apps.polyvista.com/GooglesheetFeb2021/scripts/polyvista/new223/" + selectedChart + ".png";
    }
    var tblHeader = [];
    var tblBody = [];
    if (selectedChart == "SentimentTrendChart") {
    ...
    google.script.run.withSuccessHandler(function (newlyInsertedSampleSheetDetail) {
        ...
        if (newlyInsertedSampleSheetDetail != null && newlyInsertedSampleSheetDetail.length > 0) {
            ...
            if (selectedChart == "SankeySentimentChartAdvance" || selectedChart == "SankeyNonSentimentChartAdvance") {
                ...
                AddScript("https://apps.polyvista.com/GooglesheetFeb2021/scripts/polyvista/new223/ChartExpo.js", "ChartExpo", addSampleSheetChartIntoMyChartList, true);//runInsertChartImageIntoSheetMethod, true);
            }
        }
    })
        .withFailureHandler(
...
function setDivsHeight() {
    ...
    if (!trialTopBarDisplayed) {
        ...
        $('#DataSourceDiv').height(windowHeight - trialSectionHeaderBarHeight - Heightcharticonsmenubar - defaultPaddingHeight);
        $('#DataSourceDiv .containerBody').css("max-height", windowHeight - trialSectionHeaderBarHeight - Heightcharticonsmenubar - defaultPaddingHeight - createChartButtonHeight);
    }
}
...
function convertDataIntoExcelFormat(data) {
    var convertedData = [];
    var subCategories = [];
    var row = tabularDataCharts_DimensionColumnName[selectedChart];
    var found = false;
